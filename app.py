...
st.caption("Version 6.4 • Smart-Check Logic • Library Audit Tool")
...
def get_audio(text_to_speak, slug, slow_mode=False):
    ...
    if os.path.isfile(file_path):
        # If it exists, we return it without calling Google
        with open(file_path, "rb") as f: return f.read()

    # Otherwise, generate...
    # (Stitching logic from v6.3 remains here)
    ...
    for chunk in chunks:
        response = client.models.generate_content(
            ...
            contents=f"Say this {'slowly' if slow_mode else 'clearly'} in Western Armenian: {chunk}",
    ...
    if combined_pcm:
        # Create wav and SAVE to file_path
        buf = io.BytesIO()
...
with st.sidebar:
    st.header("⚙️ Library Audit")
    existing_files = os.listdir(AUDIO_DIR) if os.path.exists(AUDIO_DIR) else []
    st.write(f"Files found in pantry: **{len(existing_files)}**")
    if st.button("🚀 Build Missing Only"):
    ...
    if existing_files:
        ...
        st.download_button("📥 Download Library ZIP", zip_buffer.getvalue(), "armenian_library.zip")
